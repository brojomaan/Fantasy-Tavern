...
namespace Coherence.Runtime
{
    ...
    {
        internal const string ConnectionReconnectPath = "/connection/reconnect";
        internal const int MaxWaitBeforeReconnectMilliSeconds = 1000;
...
#pragma warning restore CS0414
        ...
        public WebSocketManager(IRuntimeSettings runtimeSettings, RequestIdSource idSource)
        {
            ...
            AddPushCallback(ConnectionReconnectPath, OnReconnectRequested);
        }
        ...
        public void Disconnect() => DisconnectAsync().Then(task => logger.Error(Error.RuntimeWebsocketDisconnectFailed, task.Exception?.ToString(), idSource.IdBaseLogParam), TaskContinuationOptions.OnlyOnFaulted);
        ...
        internal void HandleResponse(string text)
        {
            var pos = text.IndexOf("\n", StringComparison.Ordinal);
            var metaSrc = pos == -1 ? text : text.Substring(0, pos);
            var body = pos == -1 ? null : text.Substring(pos + 1);
        ...
        private async void OnReconnectRequested(int code, string resultText, string requestId)
        {
            logger.Debug("Reconnecting...");

            await DisconnectAsync();

            // Wait for brief random amount of time to avoid a big spike from all clients reconnecting at once.
            await TimeSpan.FromMilliseconds(new Random().Next(0, MaxWaitBeforeReconnectMilliSeconds));
