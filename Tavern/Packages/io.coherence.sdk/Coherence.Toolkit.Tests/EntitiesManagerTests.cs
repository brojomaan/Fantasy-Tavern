...
namespace Coherence.Toolkit.Tests
{
    ...
    public class EntitiesManagerTests : CoherenceTest
    {
        ...
        public override void SetUp()
        {
            ...
            _ = mockConnectionManager.Setup(manager => manager.TryUpdateConnectionEntity(It.IsAny<Entity>(), It.IsAny<IncomingEntityUpdate>()))
                .Returns((Entity id, IncomingEntityUpdate update) =>
                {
                    return connectionsForNextTest.Any(conn => conn.EntityId == id);
                });
        ...
        [Test, Description("Verify that exceptions thrown during INetworkObjectInstantiator.Instantiate are caught and logged as warnings.")]
        public void Update_Catches_Exceptions()
        {
            var childEntity = entityA;
            var parentEntity = entityC;
            var componentData = new ICoherenceComponentData[] { new MockComponentData(42) };
            CreateNetworkedEntity(childEntity, componentData, parentEntity, setupSyncBuilder: x => x.NetworkObjectInstantiatorBuilder.SetInstantiateResult(() => throw new("Test Exception")));
            CreateNetworkedEntity(parentEntity, componentData, Entity.InvalidRelative, setupSyncBuilder: x => x.NetworkObjectInstantiatorBuilder.SetInstantiateResult(() => throw new("Test Exception")));

            // This should trigger INetworkObjectInstantiator.Instantiate because the child was waiting for the parent.
            entitiesManager.Update();

            Assert.That(logger.GetCountForWarningID(Warning.ToolkitSyncInstantiatorException), Is.EqualTo(1));
        }
        ...
        private ICoherenceSync CreateNetworkedEntity(Entity entityID, ICoherenceComponentData[] comps, Entity parent, ClientID? clientID = null, Action<MockSyncBuilder> setupSyncBuilder = null)
        {
            var result = CreateNetworkedEntityWithResult(entityID, comps, parent, clientID, setupSyncBuilder: setupSyncBuilder);
        ...
        private MockSyncBuilder.Result CreateNetworkedEntityWithResult(Entity entityID, ICoherenceComponentData[] comps, Entity parent,
            ClientID? clientID = null, string uuid = null, Action<MockSyncBuilder> setupSyncBuilder = null)
        {
            ...
            var networkedEntityBuilder = MockNetworkedEntityBuilder.For(entityID) // TODO: Dispose
                ...
                .ClientID(clientID)
                .UUID(uuid)
                .SetupSyncBuilder(setupSyncBuilder);
