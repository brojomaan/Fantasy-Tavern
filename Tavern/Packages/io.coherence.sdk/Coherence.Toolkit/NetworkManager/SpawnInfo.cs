...
namespace Coherence.Toolkit
{
    ...
    public struct SpawnInfo
    {
        ...
        /// <summary>
        /// The scene index of the connection entity.
        /// </summary>
        public uint? scene;
        ...
        public T GetBindingValue<T>(string bindingName)
        {
            ...
            if (binding is null)
            {
                throw new Exception($"Failed to find value binding '{bindingName}' of type {typeof(T).Name}.");
            }

            var (_, componentData) = ComponentUpdates.Store.FirstOrDefault(x => binding.CoherenceComponentType?.IsInstanceOfType(x.Value.Data) ?? false);
            if (componentData.Data is null)
            {
                throw new Exception(new StringBuilder()
                    .Append($"Failed to find component change data for value binding {binding.SignaturePlainText} of type {typeof(T).Name} among {ComponentUpdates.Count} entries.")
                    .AppendLine($"prefab: {(prefab.IsDestroyed ? assetId : prefab.name)}")
                    .AppendLine($"uniqueId: {uniqueId}")
                    .AppendLine($"ComponentUpdates: {ComponentUpdates}").ToString());
            }
