...
namespace Coherence.Toolkit
{
    ...
    public class CoherenceClientConnectionManager : IClientConnectionManager
    {
        ...
        // TODO internalize
        [EditorBrowsable(EditorBrowsableState.Never)]
        public bool TryUpdateConnectionEntity(Entity id, IncomingEntityUpdate entityUpdate)
        {
            if (!connectionsByEntityId.TryGetValue(id, out var connection))
            {
                return false;
            }

            var scene = Impl.GetConnectionScene(entityUpdate);
            if (scene.HasValue)
            {
                connection.Scene = scene.Value;
            }
