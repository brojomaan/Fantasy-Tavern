...
namespace Coherence.Toolkit
{
    ...
    public class CommandsHandler
    {
            ...
            GenericCommandRequestDelegate receiveDelegate, MessageTarget routing, Component receiver, bool usesMeta)
        {
            ...
            if (name.EndsWith("Foo")) Debug.Log($"registeredCommandNames.Add('{name}') for receiver '{receiver?.GetType().Name ?? "null"}'");
        ...
        private bool GetCommandNameAndValidateEntityId(Type targetType, string methodName, out string commandName)
        {
            ...
            while (currentType != null && !foundValidCommandName)
            {
                ...
                {
                    if (methodName.EndsWith("Foo"))
                    {
                        Debug.Log($"Command name '{commandName}' found on type '{currentType.FullName}'!");
                    }
                ...
                else
                {
                    if (methodName.EndsWith("Foo"))
                    {
                        Debug.Log($"Command name '{commandName}' not found on type '{currentType.FullName}'. Checking base type {currentType.BaseType}...");
                    }
            ...
            if (!foundValidCommandName)
            {
                ...
                string GetErrorMessage(string commandName)
                {
                    if (ReflectionUtils.TryGetMethod(targetType, methodName, BindingFlags.Public | BindingFlags.Instance, out var publicMethod))
                    {
                        ...
                        return $"Unable to send command '{commandName}' to '{Name}': command was not found. Make sure that the method {methodName} has been marked as a command on the component {Name} using the Configuration window or by adding the [Command] attribute to it. Then, Bake again (coherence > Bake menu item).";
                    }
            ...
            foreach (var command in commandData)
            {
                ...
                if (sendTo.HasFlag(SendTo.Self))
                {
                    ...
                    using (CoherenceSync.SetCurrentCommandMeta(new CommandMeta(ClientID, CurrentClientFrame)))
                    {
                        ...
                        // User-created methods could throw any exceptions.
                        catch (Exception e)
                        {
                            logger.Warning(Warning.ToolkitSyncCommandReceiveException,
                                $"An exception was thrown while executing method {commandInfo.CommandName} targeted by command: {e}",
                                ("ClientID", ClientID),
                ...
                if (sendTo.HasFlag(SendTo.Others))
                {
                    ...
                    {
                        logger.Warning(Warning.ToolkitSyncCommandSendException,
