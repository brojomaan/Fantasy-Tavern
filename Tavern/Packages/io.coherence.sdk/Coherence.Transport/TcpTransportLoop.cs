...
namespace Coherence.Transport
{
    ...
    internal class TcpTransportLoop
    {
        ...
        public const ushort MAGIC_CODE = 1337;
        public const int MAGIC_CODE_SIZE = sizeof(ushort); // 2 bytes
        public const int ROOM_UID_SIZE = sizeof(ulong);    // 8 bytes
        ...
        private async Task RunSendLoopAsync()
        {
            ...
            try
            {
                // Only send the magic code and room UID if we are connecting as a client.
                if (uniqueRoomId.HasValue)
                {
                    await SendMagicAndRoomUID(uniqueRoomId.Value);
                }
        ...
        private async Task SendMagicAndRoomUID(ulong uniqueRoomId)
        {
            var headerPacket = CreateMagicAndRoomIDPacket(uniqueRoomId);
        ...
        public static byte[] CreateMagicAndRoomIDPacket(ulong uniqueRoomId)
        {
        ...
        public static async Task<int> ReceiveFull(Stream stream, byte[] buffer, CancellationToken cancellationToken)
        {
            int totalRead = 0;
            while (!cancellationToken.IsCancellationRequested)
            {
        ...
        private async Task<int> ReceiveFull(byte[] buffer) => await ReceiveFull(stream, buffer, cancellationToken);
