    ...
    public class HttpServer
    {
        ...
        private static async Task HandleEndpointJoinRoom(HttpListenerContext ctx, HttpServer serv, Logger logger)
        {
            try
            {
                ...
                var validationResult = endpointData.ValidateLocalAddress();
                if (!validationResult.HasFlag(EndpointData.ValidationResult.ValidEndpoint))
                {
                    await Error(ctx, endpointData.GetErrorMessage(validationResult), logger);
                }
                else
                {
