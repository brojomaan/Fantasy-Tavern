namespace Coherence.Editor
{
    ...
    public static class CoherenceSyncUtils
    {
        ...
        internal static readonly string[] UndesiredOverrides =
        {
            ...
            // we don't detect archetype changes here - we need to check what exactly changed on a per-case basis
        };
        ...
        private static bool IsUndesiredOverride(PropertyModification propertyModification)
        {
            ...
            if (modificationPath.StartsWith("archetype"))
            {
                // When adding components to prefab instances, CoherenceSync will update the LOD structure (archetype).
                // When doing so, we tag as undesired only when it's not an added component override
                if (Regex.IsMatch(modificationPath , @"^archetype\.boundComponents\.Array\.data\[\d+\]\.component$"))
                {
                    var obj = propertyModification.objectReference;
                    return obj && !PrefabUtility.IsAddedComponentOverride(obj);
                }

                // Checking for changes further than this gets complex/expensive,
                // cause we need to understand what exactly has changed and why,
                // across different property modifications.
                // e.g., we don't warn about changes on Lod Steps Active
            }
