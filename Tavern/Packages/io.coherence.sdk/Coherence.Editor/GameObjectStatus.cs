namespace Coherence.Editor
{
    ...
    public readonly struct GameObjectStatus
    {
        ...
        /// <summary>
        /// Is the game object a prefab variant instance or a prefab variant open in a prefab stage?
        /// </summary>
        public readonly bool IsVariantInstance;

        /// <summary>
        /// Is the game object a prefab variant instance open in a prefab stage?
        /// </summary>
        public readonly bool IsVariantStage;
        ...
        public GameObjectStatus(GameObject gameObject)
        {
            ...
            var partOfAsset = PrefabUtility.IsPartOfPrefabAsset(gameObject);
            IsAsset = partOfAsset || IsInPrefabStage;
            IsVariantInstance = false;
            if (!partOfAsset)
            {
                var assetPath = IsInPrefabStage ? prefabStage.assetPath : PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(gameObject);
                if (assetPath != null)
                {
                    var asset = AssetDatabase.LoadAssetAtPath<GameObject>(assetPath);
                    IsVariantInstance = asset && PrefabUtility.IsPartOfVariantPrefab(asset);
                }
            }
            IsVariantStage = IsInPrefabStage && IsVariantInstance;
            IsVariantAsset = PrefabUtility.IsPartOfVariantPrefab(gameObject);
            IsVariant = IsVariantAsset || IsVariantInstance;
