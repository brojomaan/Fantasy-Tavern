namespace Coherence.Editor
{
    ...
    public class AdvancedBakeWizard : ScriptableWizard
    {
        ...
        private async void OnWizardCreate()
        {
            if (bakeUnityProject)
            {
                if (noUnityReferences)
                {
                    await BakeUtil.CustomBakeAsync(noUnityReferences, outputDirectory, waitForUpdateSyncState: ShouldWait.Never);
                }
                else
                {
                    await BakeUtil.BakeAsync(waitForUpdateSyncState: ShouldWait.Never);
                }
            ...
            new CodeGenRunner().RunAsync(codeGenData, executeInBackgroundThread: false)
                .Then(task =>
                {
                    var logger = Log.GetLogger(typeof(AdvancedBakeWizard));
                    logger.Error(Error.EditorCodeGenerationFailed,
